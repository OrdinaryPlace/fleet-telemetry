...
class Options:
    ...
    def parse(cls, value: Any) -> "Options":
        ...
        if type(port) is not int or not 1 <= port <= 65535 or type(replace) is not bool:
            raise Stop("invalid_options")
        ...
        return cls(mode, tuple(names), hostname, port, Path(ca), replace)
...
def plan_changes(selected: dict[str, str], summaries: list[dict[str, Any]],
    ...
    if any(s["configuration_present"] and not s["configuration_matches"] for s in summaries) and not replace_existing:
